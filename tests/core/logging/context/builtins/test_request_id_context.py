...
from zee_api.core.logging.context.builtins.request_id_context import (
    RequestIdContext,
)
...
def test_extract_from_request_uses_existing_header(monkeypatch):
    ctx = RequestIdContext()

    class DummyRequest:
        def __init__(self, headers) -> None:
            self.headers = headers

    req = DummyRequest(headers={"X-Request-Id": "abc-123"})
...
def test_middleware_propagates_existing_id_and_sets_response_header(
    ...
    r = client.get("/echo", headers={"X-Request-Id": hdr_val})
    ...
    assert r.headers["X-Request-Id"] == hdr_val
...
def test_middleware_generates_when_missing_and_is_unique(
    ...
    assert r1.headers["X-Request-Id"] == str(uuids[0])
    assert r2.headers["X-Request-Id"] == str(uuids[1])
