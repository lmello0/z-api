...
from zee_api.core.logging.context.builtins.trace_id_context import (
    TraceIdContext,
)
...
def test_extract_from_request_uses_existing_header(monkeypatch):
    ...
    req = DummyRequest(headers={"X-Trace-Id": "abc-123"})
...
def test_middleware_propagates_existing_id_and_sets_response_header(
    ...
    r = client.get("/echo", headers={"X-Trace-Id": hdr_val})
    ...
    assert r.headers["X-Trace-Id"] == hdr_val
...
def test_middleware_generates_when_missing_and_is_unique(
    ...
    assert r1.headers["X-Trace-Id"] == str(uuids[0])
    assert r2.headers["X-Trace-Id"] == str(uuids[1])
